...
namespace TASMod.Overlays
{
    public class TimerPanel : IOverlay
    {
        public class TimerElement
        {
            ...
            {
                if (
                    TASDateTime.CurrentFrame == FirstFrame
                    || (TASDateTime.CurrentFrame > FirstFrame && StartTime.TotalMilliseconds == 0)
                )
                {
                    StartTime = time;
                    Span = new TimeSpan();
            ...
            public void Draw(SpriteBatch spriteBatch, int left, int top)
            {
                SpriteText.drawString(
                    ...
                    2,
                    "",
                    SpriteText.getColorFromIndex(4)
                );
            }
            ...
            {
                return (span.Hours > 0) ? @"hh\:mm\:ss\.fff" : @"mm\:ss\.fff";
            }
        }
        ...
        public void RegisterTimer(
            ulong showFrame,
            ulong startFrame = 0,
            ulong endFrame = TimerElement.NO_LAST_FRAME,
            bool alwaysShow = false
        )
        {
            Timers.Add(
                new Tuple<ulong, TimerElement>(
                    showFrame,
                    new TimerElement(startFrame, endFrame, alwaysShow)
                )
            );
            Timers.Sort(
                (x, y) =>
