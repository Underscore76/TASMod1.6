...
namespace TASMod.Overlays
{
    public class WalkableLayers : IOverlay
    {
        ...
        )
        {
            ...
            foreach (Building building in location.buildings)
            {
                if (
                    ...
                    || (building.humanDoor.X < 0 && building.humanDoor.Y < 0)
                )
                    continue;

                buildingDoors.Add(
                    new Vector2(
                        building.humanDoor.X + building.tileX.Value,
                        building.humanDoor.Y + building.tileY.Value
                    )
                );
                buildingDoors.Add(
                    new Vector2(
                        building.humanDoor.X + building.tileX.Value,
                        building.humanDoor.Y + building.tileY.Value - 1
                    )
                );
            }
            ...
            {
                Rectangle tilePixels = new Rectangle(
        ...
        private readonly HashSet<string> WarpActions = new HashSet<string>
        {
            "EnterSewer",
            "LockedDoorWarp",
            "Mine",
        ...
        private readonly HashSet<string> TouchWarpActions = new HashSet<string>
        {
            "Door",
            "MagicWarp"
        };
        ...
        )
        {
            ...
            if (
                ...
                && this.WarpActions.Contains(action.ToString().Split(' ')[0])
            )
                return true;
            ...
            if (
                ...
                && this.TouchWarpActions.Contains(touchAction.ToString().Split(' ')[0])
            )
                return true;
            ...
            const int ladderID = 173,
                shaftID = 174;
            if (
                location is MineShaft
                && buildingTile != null
                && (buildingTile.TileIndex == ladderID || buildingTile.TileIndex == shaftID)
                && buildingTile.TileSheet.Id == "mine"
            )
                return true;
        ...
        private bool IsPassable(GameLocation location, Vector2 tile, Rectangle tilePixels)
        {
            ...
            // allow bridges
            if (
                location.doesTileHaveProperty((int)tile.X, (int)tile.Y, "Passable", "Buildings")
                != null
            )
            {
                Tile backTile = location
        ...
        private bool IsOccupied(GameLocation location, Vector2 tile, Rectangle tilePixels)
        {
            ...
            if (location.IsTileOccupiedBy(tile, ignorePassables: CollisionMask.Flooring))
                return true;
            ...
            {
                if (farm.resourceClumps.Any(p => p.getBoundingBox().Intersects(tilePixels)))
                    return true;
            }
