...
* frame advance (press `q` or `/` for 1 frame, hold `.` for real time)
...
Console supports scrolling, entry selection, normal copy and paste (or it should, LET ME KNOW because it works on mac fine). Console font has no support for non-ascii, so will print `?`.

**There is a basic attempt to replace your computer's username to avoid potential doxxing scenarios but you might want to try an example like `require('intential-crash')` in the lua console to see what is output. Ping me if it doesn't capture your username correctly and we can tweak the replacement.**
...
If you are using Visual Studio Code for editing files in your local `StardewTAS1.6/Scripts` folder, I recommend installing the [Lua language server](https://marketplace.visualstudio.com/items?itemName=sumneko.lua) extension to get type annotations and autocomplete for Lua. To include all the base engine lua files, open the workspace settings (Shift + Command + P (Mac) / Ctrl + Shift + P (Windows/Linux) to open the command palette) and search for `lua.workspace.library`, click Add Item, and then paste the path to the `lua` folder from the mod install.
...
#### Key Lua Helper
* `advance({keyboard={keys to press}, mouse={X=int, Y=int, left=bool, right=bool}, text=string})` - advance a frame with the given keyboard/mouse inputs. This should then pause while any existing automation is running. Example for holding down and right and clicking the mouse:
```lua
advance({keyboard={Keys.S, Keys.D}, mouse={X=100, Y=100, left=true}})
```

or for textbox input
```lua
advance({text="howdy"})
```
...
* `GameInMenu` - draw a red rectangle in top left when game is in an ItemGrabMenu
* `Grid` - draw the tile grid outlines
* `Hitbox` - draw hitboxes for all characters
* `ImGuiOverlay` - core ImGui overlay that also triggers other ImGui widgets and windows to draw
* `JotPK` - draw detailed info for Journey of the Prairie King
* `Layers` - highlight all of the unwalkable tiles
* `LuaOverlay` - draw information registered via lua script/registered lua functions to execute
...
To implement a new Overlay, create a new class that inherits from `IOverlay`. All classes that inherit from `IOverlay` will be automatically loaded and available in `OverlayManager.Get('...)`.
...
A ton of harmony patches (including patching SMAPI inputs, base System.DateTime and System.Random, the AudioEngine, network stack, etc.) to rewrap functionality and control all parts of the update/draw loop. Added some additional fields to System.Random to trace random calls for debugging purposes.
