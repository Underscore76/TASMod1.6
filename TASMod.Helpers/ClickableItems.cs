    ...
    public static class ClickableItems
    {
        ...
        public static List<ClickObject> GetClickObjects()
        {
            ...
            if (Game1.activeClickableMenu != null)
            {
                ...
                foreach (
                    var field in Reflector.GetFields<ClickableTextureComponent>(current.GetType())
                )
                {
                    ClickableTextureComponent c = (ClickableTextureComponent)
                        Reflector.GetValue(current, field);
                    if (c == null)
                        continue;
                    ClickObject clickObject = new ClickObject(c);
                    if (clickObject.Name == "-500")
                ...
                foreach (
                    var field in Reflector.GetListFields<ClickableComponent>(current.GetType())
                )
                {
                ...
                foreach (
                    string field in Reflector.GetListFields<ClickableTextureComponent>(
                        current.GetType()
                    )
                )
                {
