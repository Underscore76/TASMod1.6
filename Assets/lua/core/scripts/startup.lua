...
function startup.set_name(name)
    startup.name = name
end

---setup your farm name
---@param name string @the name you want to use
function startup.set_farm_name(name)
    startup.farm_name = name
end

---setup your favorite thing
---@param favorite string @the favorite thing you want to use
function startup.set_favorite(favorite)
    startup.favorite = favorite
end
...
---@param variant string @the variant of the shirt you want to pick
...
---@param variant string @the variant of the pants you want to pick
...
function player.get_skin()
    return Game1.player.skin.Value
end

function player.get_hair()
    return Game1.player.hair.Value
end

function player.get_shirt()
    return Game1.player.shirt.Value
end

function player.get_pant()
    return Game1.player.pants.Value
end

function player.get_acc()
    return Game1.player.accessory.Value
end

---run the startup script
function startup.run()
    if startup.name == nil or startup.farm_name == nil or startup.favorite == nil then
        error("Name, farm name, and favorite thing must be set")
    end
    ...
    advance({ text = startup.name })
    input.click_component("FarmBox")
    advance({ text = startup.farm_name })
    input.click_component("FavBox")
    advance({ text = startup.favorite })
