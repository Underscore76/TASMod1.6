...
namespace TASMod.Extensions
{
    ...
    {
        public static Dictionary<string, Dictionary<int, Tuple<bool, TimeSpan>>> DurationCache =
            new Dictionary<string, Dictionary<int, Tuple<bool, TimeSpan>>>();
        ...
        public static ConditionalWeakTable<CueWrapper, Holder> CueData = new();
        ...
        internal static Holder Initialize(this CueWrapper wrapper)
        {
            ...
            {
                XactSoundBankSound currentSound = (XactSoundBankSound)
                    Reflector.GetValue(cue, "_currentXactSound");
            ...
            if (!data.Duration.HasValue)
            {
                ...
                SoundEffectInstance soundEffectInstance = (SoundEffectInstance)
                    Reflector.GetValue(cue, "_soundEffect");
                ...
                SoundEffect effect = (SoundEffect)
                    Reflector.GetValue(soundEffectInstance, "_effect");
                data.Duration = effect.Duration;
                data.IsLooped = soundEffectInstance.LoopCount > 0;
        ...
        public static bool FrameTimeCausedStop(this CueWrapper wrapper)
        {
            var data = CueData.GetOrCreateValue(wrapper);
            return (
                data.IsInitialized && !data.IsLooped && wrapper.ElapsedTime() >= wrapper.Duration()
            );
        }
    }
}
